...
const OpenDetails = () => {
  ...
  {[
    { value: "$1.73M", title: "MRR", desc: "Monthly Recurring Revenue", bg: "#FFCECE" },
    { value: "175,514", title: "MAU", desc: "Monthly Active Users", bg: "#FFDEDE" },
    { value: "$20.78M", title: "ARR", desc: "Annual Recurring Revenue", bg: "#FFF0F0" },
    { value: "$27.81M", title: "ARPU", desc: "Average revenue per user", bg: "#FFF5F5" },
  ].map((item, index) => (
    <motion.div
      key={index}
    ...
    >
      <h4 className="w-full text-center py-10 font-semibold text-xl">
        {item.value}
      </h4>
      <div className="w-full px-10 flex flex-col items-start">
        <h5 className="font-semibold">{item.title}</h5>
        <h6 className="font-light">{item.desc}</h6>
      </div>
    </motion.div>
  ))}
</div>
