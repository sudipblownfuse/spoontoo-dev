...
const CakeArtistInfo = () => {
  ...
  return (
    <section className="w-full h-auto bg-black">
      <div className="w-full width-container mx-auto px-5 sm:px-14 flex justify-between flex-wrap py-10">
        {data.map((item, index) => (
          <div
            key={index}
          ...
          >
            ...
            {(() => {
              ...
              return (
                <h4 ref={ref} className="text-white font-bold text-2xl lg:text-3xl py-2">
                  {index === 0 ? `${count} Hours` : `${count} +`}
                </h4>
              );
            })()}

            <h6 className=" text-gray-300 text-sm sm:text-base">
              {item.heading}
            </h6>
          </div>
        ))}
